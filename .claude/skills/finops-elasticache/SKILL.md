...
description: >
  FinOps ElastiCache Analysis Skill. Detects overprovisioned elasticache nodes in AWS ElastiCache clusters using Terraform,
...
| metrics.json | CloudWatch metrics: cache_hit_rate_pct, evictions, and node count | Mark the metrics section as Not available in the provided data; verify in the real environment |
...
Flag clusters where node count is larger than usage evidence supports.
...
Waste type: Overprovisioned ElastiCache nodes

Recommended fix: Reduce node count carefully, monitor CacheHitRate and Evictions, and consider reserved nodes for the retained baseline.
...
- Modify the real main.tf so flagged replication groups use num_cache_clusters = 2, with an inline reason. Preserve unchanged resources.
