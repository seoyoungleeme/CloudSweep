...
name: finops-ebs
description: >
  FinOps EBS Snapshot Analysis Skill. Detects orphaned ebs snapshots in AWS EBS snapshots using Terraform,
...
| metrics.json | CloudWatch metrics: storage_gb per snapshot | Mark the metrics section as Not available in the provided data; verify in the real environment |
...
Flag snapshots whose SourceVolumeStatus tag is "deleted".
...
Recommended fix: Delete orphaned snapshots after verification, and prevent recurrence with Data Lifecycle Manager, automation, and tagging policy.
...
- For large orphaned snapshot sets, keep provider/terraform blocks and include a commented bulk cleanup plan plus AWS CLI deletion script. Preserve non-orphaned snapshot resources.
