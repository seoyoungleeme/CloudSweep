...
name: finops-elb
description: >
  FinOps ELB Analysis Skill. Detects idle load balancers in AWS Application Load Balancers using Terraform,
...
| metrics.json | CloudWatch metrics: request_count and active_connection_count | Mark the metrics section as Not available in the provided data; verify in the real environment |
...
Flag ALBs with no requests and no active connections over the observed period.
...
Waste type: Idle load balancers

Recommended fix: Verify DNS/listener dependencies, remove idle ALBs, then remove related security groups when safe.
...
- Comment out deletion candidates with safety checks, preserve active ALBs, and add deletion protection, idle timeout, and required tags to retained resources.
