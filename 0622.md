...
## 15. 전체 Skill 규칙을 LangGraph로 옮긴 뒤의 구조

이후 서비스 Skill이 직접 계산하던 구조도 정리했다. 핵심은 **계산하는 곳과
설명하는 곳을 분리한 것**이다.

```text
입력 파일
  → LangGraph가 도메인 감지
  → AnalyzerRegistry가 서비스 분석기 선택
  → Rule v2 검증과 Python 계산
  → cloudsweep_graph_state.json
  → Claude가 결과 검토와 설명
  → claude_review.json
  → finalizer가 최종 보고서와 Terraform 생성
```
...
| 구성요소 | 하는 일 | 하지 않는 일 |
|----------|---------|-------------|
| LangGraph | 증거 추출, 조건 판정, 절감액 계산 | 사용자에게 자연스럽게 설명하기 |
| Claude Skill | 안전성 검토, 근거 부족 설명, 문서 보강 | threshold와 절감액 다시 계산하기 |
| Finalizer | 승인 결과 합산, Terraform hash 확인 | 임의로 finding 만들기 |

### 등록된 분석 도메인

현재 18개 도메인이 같은 `analyze_domain` 노드와 `AnalyzerRegistry`를 사용한다.

```text
lambda, s3, dynamodb, bedrock, sagemaker, ec2
ebs, elb, rds
cloudwatch, cloudwatch-alarm, sqs, kinesis
ecs, elasticache
nat, tgw, organizations
```

서비스마다 LangGraph 노드를 따로 만들지 않았다. 노드가 너무 많아지는 대신,
공통 노드가 domain 이름을 보고 알맞은 분석기를 선택한다.

### Rule v2가 필요한 이유

`schemas/finops-rule-v2.schema.json`은 규칙 파일이 지켜야 할 형식이다.

```text
facts       어떤 증거를 쓰는가
predicate   all / any / not 조건을 어떻게 판정하는가
outcome     finding의 severity, action, confidence
handlers    증거 추출, 절감액, Terraform 후보를 누가 계산하는가
```

알 수 없는 fact나 handler는 실행 중간이 아니라 시작할 때 거부한다. 그래서
오타가 난 규칙이 조용히 잘못된 절감액을 만들 가능성을 줄였다.

### finding과 fact ID

같은 입력이면 `finding_id`와 `fact_id`가 항상 같게 생성된다. Claude는 이 ID를
보고 검토하므로, 어떤 근거를 승인하거나 거절했는지 추적할 수 있다.

교차 서비스 분석도 Terraform 참조 관계와 다음 지표를 fact로 저장한다.
...
Claude가 `result/claude_review.json`을 작성한 뒤 다음 명령을 실행한다.

```powershell
python -m cloudsweep finalize sample\season2\MA-001 --review sample\season2\MA-001\result\claude_review.json
```
...
`accepted` finding만 절감액에 포함된다. Terraform은 분석할 때의 원본 hash와
현재 원본 hash가 같아야 적용되므로, 중간에 사용자가 코드를 바꿨으면 안전하게
중단한다.
...
```text
Rule v2 파일 전체 검증
18개 도메인 AnalyzerRegistry 등록 검증
Season 1 전체 fixture coverage 검증
MA-001 / GENAI-001 회귀 검증
review run_id와 Terraform source hash 검증
서비스 Skill review-only 및 legacy script 제거 검증
총 25개 자동 테스트
```

한 문장으로 정리하면 다음과 같다.

> 이제 CloudSweep에서 Python/LangGraph는 계산기이고, Claude는 검토자이며, finalizer는 검토 결과를 안전하게 최종 산출물로 만드는 역할이다.

---

## 16. Claude Skill → LangGraph 하이브리드 분석 구조

### 16-1. 왜 바꿨나

15번에서 Skill을 review-only로 단순화하고 Python 분석기를 일원화했다.

그런데 복잡한 도메인(RDS, ELB, ECS, ElastiCache)의 Python 구현은 원래 SKILL.md에 있던 규칙의 일부만 구현한 stub 수준으로 줄어들었다.

예를 들어 RDS는 SKILL.md에 R1~R6 여섯 개 규칙이 있었지만 Python은 R1(비운영 Multi-AZ)과 R2(CPU 평균 <20%)만 구현했다. Extended Support 비용, Reserved Instance 커버리지, IOPS 과잉, 성능 리스크 판단(R3~R6)이 사라졌다.

ELB도 DNS 의존성 확인, blue-green 배포 감지, CLB 식별 없이 기본 idle 감지(LB1)만 남았다.

즉, 리팩터링이 규칙의 양을 줄였고, 모호한 경우를 판단하는 추론 과정도 함께 사라졌다.

Claude가 잘하는 것(복잡한 판단, 부분적인 증거 추론, 전체 규칙 적용)과 LangGraph가 잘하는 것(계산, 구조화, 재현성 보장)을 역할에 맞게 분리하기 위해 하이브리드 구조를 도입했다.

### 16-2. 도메인 분류

18개 전체 도메인을 세 종류로 나눈다.

| 종류 | 도메인 | 분석 주체 | 이유 |
|------|--------|-----------|------|
| **Simple** | lambda, s3, dynamodb, cloudwatch, cloudwatch-alarm, sqs, ebs, nat, tgw, organizations, kinesis | Python | 단일 threshold 체크이거나 boolean 조건 판정으로 충분 |
| **GenAI** | bedrock, sagemaker, ec2 | Python (rich) | B1~B4, SM1~SM4, EC2G1~EC2G4로 이미 완전하게 구현 |
| **Complex** | rds, elb, ecs, elasticache | **Claude Skill → LangGraph** | 여러 규칙 + evidence 조합 + 예외 판단이 필요 |

복잡한 도메인별 원래 규칙과 리팩터링 후 Python stub 커버리지 비교:

| 도메인 | SKILL.md 규칙 수 | 리팩터링 후 Python 구현 | 빠졌던 핵심 판단 |
|--------|:---:|:---:|---|
| rds | 6개 (R1~R6) | 2개 | Extended Support 비용, RI 커버리지, IOPS, 성능 리스크 |
| elb | 5개 (LB1~LB5) | 1개 | DNS 의존성, blue/green 감지, NLB/CLB 구분 |
| ecs | 4개 (E1~E4) | 1개 | task definition 분석, Fargate shape 검증 |
| elasticache | 5개 (EC1~EC5) | 1개 | memory p95, eviction, replication lag, HA 판단 |

### 16-3. 새 실행 흐름

```text
[복잡한 도메인: rds / elb / ecs / elasticache]

  Claude Skill (finops-rds 등)
    ├─ main.tf, metrics.json, cost_report.json 읽기
    ├─ SKILL.md 전체 규칙 + Safety Guardrail 적용
    ├─ 모호한 evidence 구간 추론
    └─ result/{domain}_skill_analysis.json 출력
                    │
                    ▼
  python -m cloudsweep <WORK_DIR>
    LangGraph analyze_domain_node
    ├─ result/{domain}_skill_analysis.json 감지 시 로드
    ├─ finding_id, savings_group, evidence_facts 부여
    └─ 파일 없으면 Python fallback stub 실행 (CLI 단독 모드)

[나머지 14개 도메인]
  Python analyzer → LangGraph 구조화  ← 변경 없음
```

```mermaid
flowchart TD
    INV[Evidence inventory] --> DET[Domain detection]
    DET --> COMPLEX[복잡 도메인\nrds / elb / ecs / elasticache]
    DET --> SIMPLE[단순·GenAI 도메인\n14개]

    COMPLEX --> SKILL[Claude Skill 실행\nfull rules + reasoning]
    SKILL --> SKILL_OUT[result/{domain}_skill_analysis.json]

    SKILL_OUT --> LG_COMPLEX[LangGraph\n_load_skill_analysis\nfinding_id 부여]
    SIMPLE --> LG_SIMPLE[LangGraph\nPython analyzer]

    LG_COMPLEX & LG_SIMPLE --> REVIEW[Claude review\ncross-domain only]
    REVIEW --> FINAL[Finalizer]
```

### 16-4. 변경한 파일

#### schemas/skill-analysis.schema.json (신규)

Claude Skill이 출력하는 `{domain}_skill_analysis.json`의 계약이다.
  ...
  "findings": [
    {
      ...
      "recommendation": "mysql-8.0으로 업그레이드해 Extended Support 비용을 제거한다.",
...
LangGraph가 나중에 부여하는 `finding_id`, `savings_group`, `evidence_facts`는 Skill 출력에 포함하지 않는다.
...
domain 필드 불일치 시 파일 전체를 무시한다. 잘못된 파일이 조용히 섞이는 것을 막기 위한 방어다.

**`_analyze_single_domain` 변경:**
...
`_registry_handler`와 `analyze_domain_node`도 `work_dir`를 context로 전달하도록 수정했다.

#### SKILL.md 복원 (4개 파일)

각 복잡 도메인의 SKILL.md에 두 가지를 복원하거나 추가했다.

1. **Detection Rules 섹션** — 원래 있던 전체 규칙 (R1~R6, LB1~LB5, E1~E4, EC1~EC5) 복원
2. **Output Contract 섹션** — `result/{domain}_skill_analysis.json` 출력 형식 명시

예시 (ElastiCache):

| 규칙 | 조건 | Severity |
|------|------|----------|
| EC1 | num_cache_clusters >2 AND p95 연결 <30% | HIGH |
| EC2 | DatabaseMemoryUsagePercentage p95 <50% AND CPU avg <20% | HIGH |
| EC3 | 예약 노드 없음 AND 월 비용 >$200 | LOW |
| EC4 | 단일 노드, 복제 없음, Multi-AZ 없음 (운영 환경) | INFO |
| EC5 | 엔진 버전이 EOL 또는 EOL 임박 | MEDIUM |

Safety Guardrail도 함께 복원했다. 예를 들어 EC1은 eviction이 0인 경우에만, EC2는 cache hit rate ≥95%인 경우에만 적용한다.

#### .claude/skills/finops/SKILL.md (오케스트레이터)

Authority Boundary 섹션을 도메인 유형별로 나눴다.

```text
Simple / GenAI 도메인: LangGraph Python이 감지·계산 소유. Claude는 검토자.
Complex 도메인: Claude Skill이 감지와 절감액 추정 소유.
               LangGraph가 finding_id, savings_group, evidence_facts 부여.
               LangGraph 실행 후에는 Claude가 savings 수치를 다시 판정하지 않는다.
```

Required Workflow를 5단계에서 8단계로 확장했다.

```text
1. WORK_DIR 아래 evidence 목록화, 존재하는 도메인 감지
2. 복잡 도메인이 있으면 각 Claude Skill을 먼저 실행
   rds → result/rds_skill_analysis.json
   elb → result/elb_skill_analysis.json
   ecs → result/ecs_skill_analysis.json
   elasticache → result/elasticache_skill_analysis.json
3. python -m cloudsweep <WORK_DIR>
   (LangGraph가 skill output 로드 + 단순 도메인 Python 분석)
4. result/cloudsweep_graph_state.json 확인
5. 각 finding 검토 (finding_id, evidence_facts 기준)
   복잡 도메인은 cross-domain 패턴과 안전 제약만 검토
6. MCP 사용 (evidence_only 또는 unavailable인 finding만)
7. result/claude_review.json 작성
8. python -m cloudsweep finalize ...
```

#### tests/test_all_domains.py

`test_service_skills_are_review_only`를 두 개로 분리했다.

| 테스트 | 검사 내용 |
|--------|-----------|
| `test_simple_and_genai_skills_are_review_only` | 복잡하지 않은 14개 Skill에 "Review-only authority"와 "LangGraph owns"가 있고 "## Detection Rules"가 없는지 확인 |
| `test_complex_skills_have_analysis_and_output_contract` | rds/elb/ecs/elasticache SKILL.md에 "Detection Rules", "Output Contract", "skill_analysis.json"이 있고 "Review-only authority"가 없는지 확인 |

신규 테스트 세 개도 추가했다.

| 테스트 | 확인 내용 |
|--------|-----------|
| `test_complex_domain_skill_output_is_loaded_by_langgraph` | `rds_skill_analysis.json`이 있으면 `_load_skill_analysis`가 올바르게 로드하는지 확인 |
| `test_skill_analysis_domain_mismatch_is_ignored` | domain 필드가 파일명과 다르면 빈 목록을 반환하는지 확인 |
| `test_complex_domain_falls_back_to_python_stub_without_skill_output` | 파일 없을 때 빈 목록 반환 → Python stub이 실행됨을 확인 |
...
| 경계 | 강제 방법 |
|------|-----------|
| `finding_id`, `savings_group`, `evidence_facts`는 LangGraph 소유 | Skill Output Contract에서 해당 필드 미포함 명시 |
| `estimated_monthly_saving_usd`는 Claude Skill 소유 | LangGraph는 이 값을 재계산하지 않음 |
| domain 불일치 파일은 무시 | `_load_skill_analysis` 내 domain 필드 검증 |
| Skill 없으면 Python fallback | 파일 없을 때 빈 목록 반환, 기존 stub 실행 |
| 단순·GenAI 도메인은 Skill에 Detection Rules 없음 | `test_simple_and_genai_skills_are_review_only` 테스트 |
...
추가된 4개 테스트:
  test_simple_and_genai_skills_are_review_only
  test_complex_skills_have_analysis_and_output_contract
  test_complex_domain_skill_output_is_loaded_by_langgraph
  test_skill_analysis_domain_mismatch_is_ignored
  test_complex_domain_falls_back_to_python_stub_without_skill_output
  ※ 기존 test_service_skills_are_review_only를 위 2개로 분리함

MA-001 dry-run: lambda, s3, dynamodb 6개 finding 유지
GENAI-001 dry-run: bedrock, sagemaker, ec2 7개 finding 유지
```
...
| | 이전 (15번 이후) | 이후 (16번) |
|---|---|---|
| 복잡 도메인 분석 | Python stub (규칙 20~33%) | Claude Skill (전체 규칙 + 추론) |
| Claude 역할 | 결과만 검토 | 복잡 도메인 분석 + 결과 검토 |
| 단순·GenAI 도메인 | Python (변경 없음) | Python (변경 없음) |
| Skill 파일 없을 때 | 해당 없음 | Python fallback (CLI 단독 실행 지원) |
| finding 수치 책임 | LangGraph Python | 복잡 도메인: Claude Skill / 나머지: Python |

> CloudSweep는 이제 "계산이 쉬운 규칙은 Python, 판단이 필요한 규칙은 Claude"로 역할을 나눠 18개 도메인 전체를 빠짐없이 분석한다.
