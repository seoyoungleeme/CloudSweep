...
CloudSweep는 로컬 rule만으로 닫힌 분석을 수행하는 것이 아니라, 연결된 MCP를 통해 가격과 공식 문서 근거를 보강한다. 특히 `aws-pricing`과 `aws-docs`는 FinOps 판단의 정확도를 높이는 핵심 연결이다.

| 연결 | 역할 |
|------|------|
| `aws-pricing` MCP | AWS Pricing API를 조회해 region-specific unit price를 확인한다. rule JSON의 static fallback 가격보다 우선 사용하며, EC2/RDS/Fargate/Lambda/NAT/TGW/S3/CloudWatch 등 서비스별 절감액 산정의 신뢰도를 높인다. |
| `aws-docs` MCP | AWS 공식 문서를 조회해 서비스 동작, 제약 조건, 과금 모델, Terraform remediation의 안전성을 검증한다. 예를 들어 S3 Gateway Endpoint region, TGW와 VPC Peering 차이, CloudWatch retention, RI/SP sharing 동작 같은 판단 근거를 보강한다. |
| GitHub API / WebFetch | 외부 문제 샘플의 README, Terraform, metrics, cost report를 가져오는 데 사용된다. `.claude/settings.local.json`에 `gh api`, `raw.githubusercontent.com`, `api.github.com` 접근 허용 내역이 있다. |
| AWS CLI | `aws configure`, `aws sts` 관련 허용 내역이 있어 실제 AWS 계정/가격/상태 검증으로 확장할 수 있는 준비가 되어 있다. |

MCP의 역할은 단순 참고 자료가 아니라 분석 신뢰도 계층을 구성하는 것이다. CloudSweep의 비용 산정 우선순위는 다음과 같다.

1. 샘플 또는 실제 워크로드의 `cost_report.json`, CUR, pricing note에 있는 직접 비용 근거를 우선한다.
2. 직접 비용 근거가 부족하면 `aws-pricing` MCP로 현재 region과 usage type에 맞는 단가를 확인한다.
3. 서비스 동작이나 제약 조건이 비용 판단에 영향을 주면 `aws-docs` MCP로 공식 문서 근거를 확인한다.
4. MCP 조회가 불가능하거나 범위 밖이면 각 `rules/*.json`의 static fallback 가격을 사용하되, 보고서에 estimate임을 명시한다.

이 연결 덕분에 CloudSweep는 "정적 규칙 기반 샘플 풀이"에 머무르지 않고, 실제 AWS 가격과 공식 서비스 동작을 반영하는 FinOps 분석 흐름으로 확장된다.
